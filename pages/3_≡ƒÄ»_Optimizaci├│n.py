...
with tab2:
    if model.order != "second":
        ...
        st.session_state["opt_results"].setdefault(response, {})["canonical"] = {
            "stationary_point": ca["stationary_point"].tolist(),
            "y_stationary": float(ca["y_stationary"]),
            "nature": ca["nature"],
        }
        st.session_state["opt_results"][response]["objetivo"] = "Maximizar" if maximize else "Minimizar"
...
with tab4:
    ...
    st.session_state["opt_results"].setdefault(response, {})["numeric"] = {
        "x_opt": result["x_opt"].tolist(),
        "y_opt": float(result["y_opt"]),
    }
    st.session_state["opt_results"][response]["objetivo"] = "Maximizar" if maximize else "Minimizar"

st.info("➡️ Continúa en **🧪 Múltiples Respuestas** para optimizar varias respuestas a la vez (deseabilidad), o ve directo a **📄 Reporte Final** para descargar un informe en PDF.")
